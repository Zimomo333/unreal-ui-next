<template>
  <div
    ref="sliderWrapper"
    class="ur-slider"
    :class="{ 'is-vertical': vertical, 'ur-slider--with-input': showInput }"
    role="slider"
    :aria-valuemin="min"
    :aria-valuemax="max"
      ...
      <template v-if="markList.length > 0">
        <div>
          <div
            ...
            class="ur-slider__stop ur-slider__marks-stop"
          >
          </div>
        </div>
        <div class="ur-slider__marks">
          <slider-marker
            v-for="(item, key) in markList"
            :key="key"
            :mark="item.mark"
...
import UrInputNumber from '@/packages/input-number'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    useWatch(props, initData, minValue, maxValue, emit, urFormItem)
...
const useWatch = (props, initData, minValue, maxValue, emit, urFormItem) => {
  ...
  const setValues = () => {
    ...
    if (props.range && Array.isArray(val)) {
      if (val[1] < props.min) {
        ...
        if (valueChanged()) {
          urFormItem.formItemMitt?.emit('el.form.change', [minValue.value, maxValue.value])
