  ...
  >
    ...
    >
      <template #default>
        <div
          ref="menu"
          :class="[`ur-menu--${mode}`, props.popperClass]"
        ...
        >
          <ul
            role="menu"
            :class="[
              'ur-menu ur-menu--popup',
              `ur-menu--popup-${data.currentPlacement}`,
            ]"
      ...
      <template #trigger>
        ...
        >
          ...
          <i :class="['ur-submenu__icon-arrow', submenuTitleIcon]"></i>
        </div>
      </template>
    </ur-popper>
    <div
    ...
    <ur-collapse-transition v-if="!isMenuPopup">
      <ul
        v-show="opened"
        role="menu"
        class="ur-menu ur-menu--inline"
        :style="{ backgroundColor: rootProps.backgroundColor || '' }"
      >
        <slot></slot>
      </ul>
    </ur-collapse-transition>
  </li>
</template> -->
...
import UrCollapseTransition from './collapse-transition.vue'
...
export default defineComponent({
  ...
  setup(props: ISubmenuProps) {
    ...
    const isFirstLevel = computed(() => {
      ...
      while (parent && parent.type.name !== 'UrMenu') {
        if (['UrSubmenu', 'UrMenuItemGroup'].includes(parent.type.name)) {
          isFirstLevel = false
          break
        } else {
    ...
    const menuTransitionName = computed(() => {
      return rootProps.collapse ? 'ur-zoom-in-left' : 'ur-zoom-in-top'
    })
    ...
    const handleMouseleave = (deepDispatch = false) => {
      ...
      if (appendToBody.value && deepDispatch) {
        if (instance.parent.type.name === 'UrSubmenu') {
  ...
  render() {
    ...
    const titleTag = [
      this.$slots.title?.(),
      h('i', {
        class: ['ur-submenu__icon-arrow', this.submenuTitleIcon],
      }, h('svg', {
        t: '1620476572274',
        class: 'icon',
    ...
    // temporaryly mark UrPopper as any due to type inconsistency.
      ...
      ? h(UrPopper as any, {
        ...
        default: () => h('div', {
          ...
          h('ul', {
            class: [
              'ur-menu ur-menu--popup',
              `ur-menu--popup-${this.data.currentPlacement}`,
            ],
            style: ulStyle,
          }, [this.$slots.default?.()]),
        ]),
