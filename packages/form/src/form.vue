<template>
  <form
    class="ur-form"
    :class="[
      labelPosition ? 'ur-form--labur-' + labelPosition : '',
      { 'ur-form--inline': inline }
    ]"
  >
    <slot></slot>
  </form>
...
import {
  urFormKey, UrFormItemContext as FormItemCtx,
  urFormEvents, ValidateFieldCallback,
} from './token'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    formMitt.on<FormItemCtx>(urFormEvents.addField, field => {
      if (field) {
        fields.push(field)
      }
    })

    formMitt.on<FormItemCtx>(urFormEvents.removeField, field => {
      if (field.prop) {
    ...
    const resetFields = () => {
      if (!props.model) {
        console.warn(
          '[Warn][Form]model is required for resetFields to work.',
        )
        return
      }
      fields.forEach(field => {
