...
import { generateId, useGlobalConfig } from '@/utils/util'
...
export const useDropdown = () => {
  ...
  const urDropdown = inject<IElDropdownInstance>('urDropdown', {})
  const _urDropdownSize = computed(() => urDropdown?.dropdownSize)
...
export const initDropdownDomEvent = (dropdownChildren, triggerElm, _instance) => {
  ...
  function initAria() {
    ...
    if (!_instance.props.splitButton) {
      ...
      addClass(triggerElm, 'ur-dropdown-selfdefine')
    }
  }

  function initEvent() {
    on(triggerElm.value, 'keydown', handleTriggerKeyDown)
