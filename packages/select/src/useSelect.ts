...
export function useSelectStates(props) {
  ...
  return reactive({
    ...
    inputHovering: false,
    cachedPlaceHolder: '',
    currentPlaceholder: t('ur.select.placeholder'),
...
export const useSelect = (props, states: States, ctx) => {
  ...
  const selectDisabled = computed(() => props.disabled || urForm.disabled)
  ...
  const iconClass = computed(() => props.remote && props.filterable ? '' : (states.visible ? 'arrow-down' : 'arrow-up'))

  const debounce = computed(() => props.remote ? 300 : 0)

  const emptyText = computed(() => {
    if (props.loading) {
      return props.loadingText || t('ur.select.loading')
    } else {
      if (props.remote && states.query === '' && states.options.size === 0) return false
      if (props.filterable && states.query && states.options.size > 0 && states.filteredOptionsCount === 0) {
        return props.noMatchText || t('ur.select.noMatch')
      }
      if (states.options.size === 0) {
        return props.noDataText || t('ur.select.noData')
      }
    }
    return null
  })
  ...
  const selectSize = computed(() => props.size || urFormItem.size || ELEMENT.size)
    ...
    if (!val) {
      ...
      if (props.filterable) {
        ...
        if (props.multiple) {
          ...
          if (!props.remote) {
            states.selectEmitter.emit('urOptionQueryChange', '')
            states.selectEmitter.emit('urOptionGroupQueryChange')
          }
        }
        handleQueryChange(states.query)
      }
  ...
  const handleQueryChange = val => {
    ...
    if (props.remote && typeof props.remoteMethod === 'function') {
      ...
      states.filteredOptionsCount = states.optionsCount
      states.selectEmitter.emit('urOptionQueryChange', val)
      states.selectEmitter.emit('urOptionGroupQueryChange')
    }
    if (props.defaultFirstOption && (props.filterable || props.remote) && states.filteredOptionsCount) {
      checkDefaultFirstOption()
